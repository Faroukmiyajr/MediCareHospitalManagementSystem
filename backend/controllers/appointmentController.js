...
export const createAppointment = async (req, res) => {
  try {
    ...
    if(!doctorId || !patientName || !mobile || !date || !time || !fee){
        return res.status(400).json({
            success: false,
...
if (existingAppointment) {
    return res.status(400).json({
        success: false,
        message: "You have already booked an appointment with this doctor at the same date and time."
    });
}
let doctor = null;
try {
    ...
    console.warn("Doctor not found for appointment creation:", e?.message|| e);
}
...
export const confirmPayment = async (req, res) => {
  try {
    ...
    console.error('Confirm Appointment unexpected:', confirmerror);
    return res.status(500).json({
...
export const updateAppointment = async (req,res)=> {
    ...
    if (body.date && body.time) {
      ...
      updateData.rescheduledTo = { date: body.date, time: body.time };
    }
const updated = await Appointment.findByIdAndUpdate(id, updateData,
    ...
    console.error('Updated Appointment error.', error);
    return res.status(500).json({
        success: false, message: 'server error'
    })
  }
