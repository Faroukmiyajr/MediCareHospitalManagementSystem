...
export const createServiceAppointment = async (req, res) => {
  try {
    ...
    const svcImageUrlFromDB =
      svc &&
      (String(svc.imageUrl || svc.image || svc.image?.url || svc.profileImage?.url || '').trim() || '');
...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    if (!session) {
      return res.status(404).json({ success: false, message: 'Invalid session_id' });
    }
...
export const updateServiceAppointment = async (req, res) => {
  try {
    ...
    if (updateData.payment) {
      ...
      if (updateData.payment.status && updateData.payment.status === 'Confirmed') {
...
// to getAppointmentForThePatient
